...
> 基于 B 站「黑马程序员 Python+AI 零基础入门到大神」课程（BV1sHU9BmEne，185集）的系统化学习仓库，配合 Claude Code 智能体实现"看视频 → 提问 → 生成笔记 → 每日复盘"的闭环学习流程。
...
### 2. 可运行示例代码（`notes/*/examples/`）

每个知识点配 1 个最小可运行 `.py` 示例，要求：

- 中文注释、数字前缀排序、文件头标注生成时间
- **只用已学语法**（学函数前不用 `def`，学 OOP 前不用 `class`）
- 优秀的学习者实战案例（如购物车、教务系统）直接收录

### 3. 每日复盘报告（`daily_summaries/`）

每天学习后生成双文件复盘：

- `summary_YYYY-MM-DD.md`：9 段完整报告（知识覆盖/代码质量/难度评估/掌握度/坑点/学习建议）
- `practice_YYYY-MM-DD.py`：纯注释练习题（巩固练习 + 自测题，留 `# 我的回答：` 待填）

### 4. AI 辅助工具链（`.cursor/skills/` + `.claude/agents/`）

| 工具 | 文件 | 作用 |
|------|------|------|
| **学习 Skill** | `.cursor/skills/learn-python-beginner/SKILL.md` | 定义学习路径、Java 对照表、笔记模板、生成笔记命令 |
| **学习分析师** | `.claude/agents/learning-analyst.md` | 分析每日笔记+代码，生成复盘报告和练习题 |
...
├── daily_summaries/
│   └── 2026-MM-DD/                       # 每日复盘（summary + practice）
...
如果你也用 Claude Code 学习，Skill 和 Agent 已内置在仓库中：
